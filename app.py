...
app.config['JWT_SECRET_KEY'] = "lear_with_Pratap"
...
def expired_token_callback(jwt_header, jwt_payload):
    return (jsonify({"message":"The provided token has expired", "error":"token expired"}), 401)
...
def missing_token_callback(error):
    return (jsonify({"message":"Message dows not contain an access token", "error":"authoritazion required"}), 401)
